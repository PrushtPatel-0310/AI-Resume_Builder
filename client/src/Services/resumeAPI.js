...
const getHybridAtsScore = async (payload) => {
  try {
    const response = await axiosInstance.post("resumes/ats-score", payload);
    return response.data;
  } catch (error) {
    throw new Error(
      error?.response?.data?.message || error?.message || "Something Went Wrong"
...
const generateAiContent = async (payload) => {
  try {
    const response = await axiosInstance.post("resumes/ai-generate", payload);
    return response.data;
  } catch (error) {
    throw new Error(
      error?.response?.data?.message || error?.message || "Something Went Wrong"
...
const parseResumePdfFile = async (file) => {
  try {
    const formData = new FormData();
    formData.append("file", file);

    const response = await axiosInstance.post("resume/parse", formData, {
      headers: {
